...
## 2026-08-12 — M5 planning pass: the crate boundary the plan had wrong, the half of a terminal nobody had written down, and five things I asserted about code I had read
...
**This entry was revised before it landed.** Its review (PR #186) found five factual claims in
it that were not true of the code, two of them load-bearing — the whole prerequisites section
rested on one. The entry is corrected in place rather than followed by an erratum, because it
had not yet entered the record: the append-only rule protects reasoning that *was* true on its
date from being rewritten by later knowledge, and nothing here was ever true. What the review
found is recorded at the end, because the pattern in it is more useful than the corrections.
...
modifiers }` and `nxsh` expects bytes.

The gap is narrower than the first draft of this entry said. `libinput::keymap::to_char`
already folds control to the C0 range — `Ctrl-C` is `0x03`, and its doc says why. What is
genuinely missing is the arrows, Home, End and Delete (absent from the `US` table, and escape
*sequences* rather than characters, so they could not live in a `keycode → u8` function
anyway), Backspace, and Enter, which is encoded `\n` where a terminal wants `\r`. So the
encoder **delegates** for the text-and-control half rather than reimplementing a C0 fold that
...
than each against my belief about it — which is worth having and is **not** coverage of the
encoder. `Discipline::Key` is exactly `Up` and `Down`, so the round-trip reaches two of the
several keys A6 adds, and Home, End and Delete still need assertions of their own.
...
### One prerequisite, and the two that were not

Before Part A: the doc graduation M4 left. That is all.

The first draft listed two more — **window ids on `KeyEvent`/`PointerEvent`** and **popup
placement** — and justified both on one premise: that the terminal's menu is a compositor-level
popup *window*. **The toolkit says it is not**, in the very widget M5 will use:

> The popup half of a menu is not here: an open menu is a `stack` layer the application adds
> over its content, which needs the popup positioned under its item and is Milestone 5's first
> real requirement for it.

`widget-toolkit.md` §5 agrees, giving `stack`'s reason for existing as "menu popups over the
grid". So `nxterm` creates exactly one window, the "first client with two windows" trigger does
not fire, and `CreateWindow` positioning is not what places the menu. What M5 needs is an
*offset within a stack* — a `libui` change, in Part B.

The mistake is worth naming precisely, because it is not "I was wrong about a detail". I
justified two **wire breaks** on a claim about a component I had read, and the component's own
doc-comment contradicted it. The plan then read as rigorous — filed triggers quoted verbatim,
Wayland's positioner design weighed against a cheap alternative — while resting on a premise
nobody had checked. A trigger-based discipline is only as good as the check that the trigger
fired.

**Where those two now stand.** The window id stays deferred with a corrected trigger: the first
client that genuinely holds two compositor windows, which is a `Role::Dialog` or a popup that
must escape its parent — M6 or M7. Popup placement stays unbuilt and unneeded.

**And the boundary no document stated**, which is the useful thing that came out of getting it
wrong: `rsproto-surface-ops.md` says a popup "may extend beyond its parent's bounds; a menu
clipped to its window is not a menu", while the toolkit makes M5's menu an in-window layer that
is clipped by construction. Both are right for their case. **A menu that fits inside its window
is a `stack` layer; a menu that must escape it is a `Role::Popup`.** M5's terminal is 80×24
with a bar along its top, so its dropdowns fit.
...
### What the review found, and the one pattern in it

Five factual claims in the first draft were wrong. Listed together because they have a single
cause and it is worth being able to recognise:

1. `KeyEvent` is **8 bytes**, not 12 — so a wire break was mis-sized at both ends, and the
   "spare bytes" it assumed were two rather than four.
2. **The terminal's menu is not a second window** — the premise under two prerequisites.
3. `keymap::to_char` **already** folds control to C0 and encodes Enter.
4. Key repeat **already has a consumer and a gate**; the plan said it had neither.
5. The deferrals table still pointed `TODO(history-pager)` at M4 — in the same pass that
   marked M4 complete and moved cursor addressing to M5, 250 lines apart in one file.

**Every one is a claim about code I had read.** Not a guess, not an area I had skipped — the
`menu_bar` doc-comment that refutes claim 2 is four lines above the function M5 is built on,
and I had read it in the same session that wrote the plan. What produced the errors was
writing from a *model* of the code rather than from the code, at exactly the moment the model
felt reliable enough not to check.

This is the same failure as the milestone's recurring test problem — "the test observed
something downstream of what it claimed to check" — moved from tests to prose. A planning
document's whole value is that its factual claims are load-bearing for work that has not
started, so nothing downstream will catch them. Here the reviewer did, before any of it was
built, which is the argument for planning passes going through review at all.
