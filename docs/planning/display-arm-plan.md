...
- [ ] **New Window in all three.** One process, several top-level windows — the protocol allows
      ...
      - [ ] **The conversion**, which is the large half and is *uniform or nothing*: if the first
            window stays the loop's own and the extras are `Child`s, closing the first kills the
            rest — worse than not having New Window at all. `nxedit` and `nxfiles` convert
            directly; **`nxterm` needs one more toolkit seam**, because `Child::present` paints
            with an empty custom callback and `nxterm`'s grid is a `custom` node.
- [ ] **Quit**, per decision 4 — which is now settled, and lands with New Window because it is
      not observable before it.
