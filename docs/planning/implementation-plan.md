...
| — Administration | [administration.md](administration.md) | in progress — seven parts (A–G); **Part A complete 2026-09-23** (views, the broker and `with`); **Part B detailed 2026-09-23** and **complete 2026-09-24** (the device manager: `/dev/registry`, `device-mgr`, `input-server` from the manager, `/dev/devices` in every session, the probes, and [`device-manager.md`](../architecture/device-manager.md)); **Part C detailed 2026-09-24** (storage); **C.1 complete 2026-09-24** (one page-cache object per file, dirty ones kept, `sys_ns_sync`, `File::Forget`); **C.2 complete 2026-09-24** (`IoOpcode::Flush`, AHCI's `FLUSH CACHE EXT`); **C.3 complete 2026-09-24** (read-only mounts, `s_state`, `Meta::Unmount`); **C.4 complete 2026-09-24** (`OBJECT_KIND_SUBNAMESPACE`); **C.5a complete 2026-09-25** (`storage-service` owns `block`, reports what each disk holds and which are `init`'s, and serves the table; [`storage.md`](../architecture/storage.md)); **C.5b complete 2026-09-25** (auto-mount, labels, a namespace per mount, `SUBNAMESPACE` through the service, the session endpoint, and a scratch disk in every test image); **C.5 complete 2026-09-25** with C.5c (the admin endpoint, `Storage` — `Mount`, `Unmount`, `InUse` — the unmount chain, and `sys_ns_held`); **C.6 complete 2026-09-25** (`/storage` and `/dev/storage` in every session and application; the `storage` grant; `disks` withholding what is in use); **C.7 complete 2026-09-25** (`disk`: `--list` from any session, `--mount` and `--unmount` through the `storage` grant); **Part C complete 2026-09-25** with C.8 (`check-storage`: the test live image beside a copy of the release disk, a file written through a mapping without a sync, unmounted, and checked on the host); **Part D detailed 2026-09-25** (accounts); **D.1 complete 2026-09-25** (`libusers`, and `auth-service`'s admin session: `List`, `Add`, `Remove`, `SetPassword`, written atomically); **D.2 complete 2026-09-25** (the `views` grant and the policy endpoint: `with --show`, `with --install`, and an administrator as whoever can use `views` for every program); **D.3 complete 2026-09-25** (the broker fronts accounts: the `accounts` grant's `AddAccount`, `RemoveAccount` and `SetPassword`, homes made with their folders, `Accounts` and a paced `ChangePassword` for every session, and removal waiting for logout and an administrator); **D.4 complete 2026-09-25** (`account`: the list and your own password from any session, adding, removing and setting passwords through the `accounts` grant, and an offline mode on a users file for recovery); **Part D complete 2026-09-25** with D.5 (`check-recovery`: a password reset from the live image on an installed disk, which then logs in with it) |
...
- **Phase 5 (Bare metal):** 🚧 **active from 2026-09-10** — the first phase whose goal is a
  ...
  - **Part H.1 (the installer) is complete, 2026-09-16: the system installs itself.** The live
    ...
    no authority of its own — refuses anything that is not a whole disk, requires the disk's
    identity typed back as an *argument* (nothing in this system reads a terminal), and copies the
    two sources it finds by what they contain. `cargo xtask check-install` drives the whole path a
