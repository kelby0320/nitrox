...
**The live image's copy has a menu** (Phase 5 Part D), generated by `cargo xtask image --live`
from this one: `timeout: 5`, the entry above first — so the countdown boots it — and a second,
`Nitrox — hardware report`, identical but for `cmdline: hwreport`. Only the live image carries it,
because it is the stick a person boots an unfamiliar machine with; no other image pays for a
countdown.
...
3. **What was handed over, and what this CPU is** (Phase 5 Part D, 2026-09-14). `log_handoff`
   ...
   and a word it does not know is logged and ignored, never fatal;
