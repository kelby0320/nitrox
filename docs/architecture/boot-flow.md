...
`module_path: boot():/boot/initramfs`. Timeout is 0 so the entry boots immediately. **A test
image's copy loads one more module**, `boot():/boot/scratch.img`, which the kernel publishes as a
RAM disk for the storage service to mount (administration Part C.5b;
