...
middle-click within five seconds of the first* since M12 Part A: a shell cannot tell a client
that is wedged from one that is asking its user a question, and a timer against the latter
destroys the work the question was about. A client must be correct under either, which is what
"cannot tell how long it has" means — and under a third policy nobody has written yet, which is
why this paragraph describes one shell rather than constraining any.
