...
//!
//! Like `nxfiles`, `nxinstall` is also a **library** with host unit tests
//! (`cargo test -p nxinstall --lib`). The fixed-address bare-target script must NOT reach that
//! host link (it corrupts it — the linker errors), so we use `rustc-link-arg-bins`, which
//! applies only to the `[[bin]]`, never to the lib test binary.
