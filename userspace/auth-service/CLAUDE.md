...
client meant to hold one, and does from Part D.2 — `List` to judge a policy, and since D.3 every
account write a person asks it for; the root namespace can reach it too, the boundary `/svc/auth` has always had (`TODO(svc-auth-ungated)`).
